...
void DG_DrawFrame()
{
    ...
    s_FrameCount++;
    if (s_FrameCount==35 || s_FrameCount==70 || s_FrameCount==105 || s_FrameCount==150 || s_FrameCount==300) {
        char fname[64]; snprintf(fname, sizeof(fname), "evidence/DOOM_NATIVE_BASELINE/frame_%d.raw", s_FrameCount);
        FILE* f=fopen(fname,"wb");
        if (f) { fwrite(DG_ScreenBuffer, 4, DOOMGENERIC_RESX*DOOMGENERIC_RESY, f); fclose(f); printf("[capture] frame %d dumped %s\n", s_FrameCount, fname); }
        snprintf(fname, sizeof(fname), "evidence/DOOM_NATIVE_BASELINE/frame_%d.sha", s_FrameCount);
        // también hacer hash via external, pero dump ya
    }
